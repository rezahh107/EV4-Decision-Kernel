...
const impactText = read('planning/coverage/impacts/dcov-exec-001.aigov-v3-batch-b.json');
...
if (!new RegExp(`frozen_plan:\\s*${PLAN_ID}`).test(nextWork)) fail('planning/NEXT_WORK.md', 'V3 plan identity is missing');
if (!/repository_adoption_status:\s*pending_batch_b_exact_main_completion/.test(nextWork)) fail('planning/NEXT_WORK.md', 'PR head overstates or omits pending Batch B exact-main status');
if (!/BATCH_A:\s*exact_main_reconciled_under_v3_exception/.test(nextWork)
  || !/AIGOV-ADOPT-000_through_007:\s*merged_and_post_merge_reconciled/.test(nextWork)) fail('planning/NEXT_WORK.md', 'Batch A V3 reconciliation state is missing');
if (!/batch:\s*BATCH_B/.test(nextWork)
  || !/increment:\s*AIGOV-ADOPT-008/.test(nextWork)
  || !/status:\s*implemented_pending_exact_head_validation_and_review/.test(nextWork)) fail('planning/NEXT_WORK.md', 'Batch B active state is incorrect');
if (!/KREC-001_through_009:\s*registered_planned_task/.test(nextWork)) fail('planning/NEXT_WORK.md', 'KREC registration state is missing');
if (!/KROAD-012:\s*next_product_task_blocked_pending_final_aigov_closure/.test(nextWork)
  || !/KROAD-013_through_018:\s*not_started/.test(nextWork)
  || !/KROAD-012R:\s*historical_non_authoritative/.test(nextWork)) fail('planning/NEXT_WORK.md', 'KROAD preservation state is incorrect');
if (!/status:\s*not_measurable_pending_external_promotion/.test(nextWork)
  || !/percentages:\s*null/.test(nextWork)
  || !/coverage_promotion_effect:\s*none/.test(nextWork)
  || !/product_effect:\s*none/.test(nextWork)) fail('planning/NEXT_WORK.md', 'Coverage or product effects are overstated');
const nextTask = section(nextWork, 'Next Task');
const unchecked = [...nextTask.matchAll(/^- \[ \] ((?:KROAD|DCOV-EXEC|AIGOV-ADOPT)-\d{3})\s+—/gm)];
if (unchecked.length !== 1 || unchecked[0]?.[1] !== 'AIGOV-ADOPT-008') fail('planning/NEXT_WORK.md', 'exactly one current task, AIGOV-ADOPT-008, is required');
...
if (!new RegExp(`plan_id:\\s*${PLAN_ID}`).test(decision)
  || !/plan_version:\s*3/.test(decision)
  || !new RegExp(`audit_base_sha:\\s*${BASE_SHA}`).test(decision)
  || !/previous_plan_id:\s*GOV-ADOPTION-EV4-DECISION-KERNEL-5FF5D7B-V2/.test(decision)) fail('planning/decisions/AIGOV_ADOPTION_DECISION.md', 'authoritative V3 decision identity is incomplete');
if (!/one-time/i.test(decision) || !/impossible_retrospective_review_cycle/.test(decision) || !/owner_only/.test(decision)) fail('planning/decisions/AIGOV_ADOPTION_DECISION.md', 'V3 exception or owner-only Merge boundary is missing');
if (!new RegExp(`plan_id:\\s*${PLAN_ID}`).test(audit)
  || !/repository_adoption_status:\s*pending_batch_b_exact_main_completion/.test(audit)
  || !/No historical independent Green receipt is claimed/i.test(audit)) fail('planning/reviews/AIGOV_ADOPTION_AUDIT.md', 'audit identity or evidence limit is incorrect');
...
let impact = null;
try { impact = JSON.parse(impactText); } catch (error) { fail('planning/coverage/impacts/dcov-exec-001.aigov-v3-batch-b.json', `invalid JSON: ${error.message}`); }
if (impact && (impact.pull_request !== 50
  ...
  || impact.zero_delta !== true)) fail('planning/coverage/impacts/dcov-exec-001.aigov-v3-batch-b.json', 'Coverage impact is not a zero-effect PR #50 record');
...
const parts = [
  ['part-001-01.b64', '548b94eceb853d87fb8387b8ba1b96464bd495a6eb6654559d2f50cdfbfabd3a'],
  ['part-001-02.b64', '9f99aa4b9d951e76eec4694f8a3c2c917e5b377b0b1dfa5372e07b9533f9920d'],
  ['part-001-03.b64', '020b5f2a6a0d8b757307530278301e1720bb4cbbcef335e85826c5f889222979'],
  ['part-001-04.b64', 'c2a19c5357790034a9fca86402cda76ed577917396ac2ad731cdbb2714f6d125'],
  ['part-001-05.b64', '53e471edeb4a696561009a2245002b8439084ad560b57a969cda49e253de484c'],
  ['part-001-06.b64', 'eeaef0e374959cb2b9a0d2d138f511008d66f0c740c2b20240050db57c1370de'],
  ...
  ['part-004.b64', '1701611223e4561456569b09a517751c03a48fb487685387fad50e1da2df7c26'],
  ['part-005-01.b64', '2e7145e45488ba7da40a7c3c486c317d024ba611d51c1efab7fb34dbd8995dbd'],
  ['part-005-02.b64', '6ad0b4c65345e217eaa7942a74662e0b833b37c45ebe8971554ed1467a7a647f'],
  ['part-005-03.b64', '313b891ce7d4101f4158a3920fc7070394958eeba4a35a7de1b93592a8e8a70d'],
  ['part-005-04.b64', '7b74515dafb67d9b76ac2ee6b5203cdb4332851f91ce1f72eaf14312794c7e00'],
  ['part-005-05.b64', '5e8f923d78729eaf0f19cfe7a3a9615644dafe65c0ec35cdc851d524fcbe4adf'],
  ['part-005-06.b64', 'b6458ca06b7b87186275657480c909c6c8e9793b5a60e423acbbbda20da9942e'],
  ['part-006.b64', 'f578e37ca4b9e29fb8505eba97c4d357a6510b7afd2a0ecdbebf0032c060bf43'],
  ['part-007.b64', 'ef29003748aee9069ce3975a5abcc313bc63fd84d61a9b714609fcf18a2dd9ab'],
