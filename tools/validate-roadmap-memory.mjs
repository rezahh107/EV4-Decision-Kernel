...
function assertHistoryMatrixCompatibility() {
  ...
  const isV3 = nextWork.includes(V3_PLAN_ID);
  if (isV3) {
    if (!/^- \[ \] AIGOV-ADOPT-008\s+—\s+Final AIGOV exact-main closure \(`BATCH_B`\)$/m.test(next)
      || !/`status`:\s+`implemented_pending_exact_head_validation_and_review`/.test(next)) failures.push('The V3 Batch B carrier must remain the single current governance task.');
    if (!/^- \[ \] KROAD-012\s+—\s+External Evidence Producer Boundary$/m.test(nextProduct)
      || !/`status`:\s+`next_product_task_blocked_pending_final_aigov_closure`/.test(nextProduct)) failures.push('KROAD-012 must remain preserved behind final AIGOV closure.');
  } else {
    if (!/^- \[ \] AIGOV-ADOPT-001\s+—\s+AIGOV Enforcement Activation \(`BATCH_A`\)$/m.test(next)
      || !/`status`:\s+`in_batch_a_implementation`/.test(next)) failures.push('The V2 Batch A enforcement carrier must remain the single current governance task.');
    if (!/^- \[ \] KROAD-012\s+—\s+External Evidence Producer Boundary$/m.test(nextProduct)
      || !/`status`:\s+`next_product_task_blocked_by_governance_adoption`/.test(nextProduct)) failures.push('KROAD-012 must remain the preserved next product task.');
  }
  if (!/^- \[ \] `?DCOV-EXEC-001`?\s+—\s+Coverage Guarantee proposal and validation foundation$/m.test(coverage)
    || !/`implementation_eligibility`:\s+`blocked_pending_external_governance_approval`/.test(coverage)) failures.push('DCOV-EXEC-001 must remain a blocked, non-executable proposal.');
  ...
  if (!/\*\*Status:\*\* proposed/.test(executionPlan)
    ...
    || /replaces KROAD-012 through KROAD-018/.test(executionPlan)) failures.push('The detailed plan must keep the Coverage overlay proposed and non-executable.');
...
} else if (nextWork.includes(V3_PLAN_ID)) {
  await import('./validate-roadmap-memory-v3.mjs');
} else {
