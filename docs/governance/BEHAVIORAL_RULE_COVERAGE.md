...
| `AIGOV-SCOPE-001` | Scope lifecycle preserves committed, excluded, deferred and dependency sets across turns. | Critical | docs/governance/AI_AUTHORITY_DETERMINISTIC_GOVERNANCE_SSOT_v1.1.0.fa.md | aigov-scope-manifest.v1.schema.json | validate-aigov-governance.mjs and test-aigov-sequence.mjs reject deleted deferred items and cross-turn mutation | material-l3.json | deferred-item-deleted.json, cross-turn-mutation.json | AIGOV sequence gate / npm run test:aigov-sequence | None | cross_turn | block | `sequence_ci_enforced` |
| `AIGOV-SCOPE-DISCLOSURE-001` | Computed base-to-head paths must exactly equal the declared committed scope. | Critical | docs/governance/AIGOV_EXACT_MAIN_CLOSURE_PROTOCOL.md | aigov-scope-manifest.v1.schema.json | generate-aigov-scope-disclosure.mjs compares the exact Git diff and scope revision | material-l3.json | deferred-item-deleted.json, stale-scope-revision.json | AIGOV exact-head scope disclosure | None | cross_turn | block | `sequence_ci_enforced` |
| `AIGOV-PROGRESS-001` | Completion cannot be asserted before the ordered exact-main receipt. | Critical | docs/governance/AIGOV_EXACT_MAIN_CLOSURE_PROTOCOL.md | aigov-evidence-manifest.v1.schema.json completion_receipt | validate-aigov-governance.mjs emits AIGOV_PREMAIN_COMPLETION_FORBIDDEN and sequence diagnostics | material-l3.json | missing-required-carriers.json, merge-before-review.json | AIGOV sequence gate / npm run test:aigov-sequence | None | cross_turn | block | `sequence_ci_enforced` |
| `AIGOV-EVIDENCE-001` | Evidence manifests bind exact base, head, scope revision, budgets and pending completion state. | Critical | docs/governance/AIGOV_EXACT_MAIN_CLOSURE_PROTOCOL.md | aigov-evidence-manifest.v1.schema.json | validate-aigov-governance.mjs compiles and validates the manifest schema and Batch A carrier | material-l3.json | missing-required-carriers.json | AIGOV exact-head gate / npm run validate:aigov | None | per_artifact | block | `ci_enforced` |
| `AIGOV-INDEPENDENCE-001` | The implementation identity cannot issue its own independent verdict. | Critical | docs/governance/AIGOV_REVIEW_COMPATIBILITY_PROFILE.md | aigov-review-receipt.v1.schema.json | validate-aigov-governance.mjs rejects identical reviewer and implementer identities | material-l3.json | self-review.json | AIGOV fixture gate; external receipt remains required before owner Merge | None | cross_turn | block | `fixture_tested` |
| `AIGOV-STALE-001` | Any head or scope revision mutation invalidates earlier review evidence. | Critical | docs/governance/AIGOV_REVIEW_COMPATIBILITY_PROFILE.md | aigov-review-receipt.v1.schema.json head_sha and scope_revision | validate-aigov-governance.mjs rejects stale head and stale scope receipts | material-l3.json | stale-head.json, stale-scope-revision.json | AIGOV exact-head and sequence gates | None | cross_turn | block | `sequence_ci_enforced` |
| `AIGOV-MERGE-001` | Independent review and owner-only Merge cannot be skipped in the Batch sequence. | Critical | docs/governance/AIGOV_EXACT_MAIN_CLOSURE_PROTOCOL.md | aigov-repository-policy.v1 sequence and authority | test-aigov-sequence.mjs rejects merge-before-review and exact-main verifier requires the owner actor | material-l3.json | merge-before-review.json | AIGOV sequence gate / npm run test:aigov-sequence | None | cross_turn | block | `sequence_ci_enforced` |
...
| `AIGOV-SECURITY-PROFILE-001` | Secrets, permissions, rulesets, history, external writes and broad dependency changes remain forbidden. | High | kernel/decision-governance/aigov-repository-policy.v1.json | aigov-repository-policy.v1 security_profile | validate-aigov-governance.mjs inspects workflow diffs, action pins, dependency versions and deletion paths | material-l3.json | security-and-budget-weakening.json | AIGOV exact-head gate / npm run validate:aigov | None | per_artifact | block | `ci_enforced` |
