...
- [x] KROAD-010 — Downstream Consumer Contract
  ...
  - Fourth-review anchor note: The final acceptance stack is stored in non-self-referential anchor commit `e298c2544d16156a813af3be2f32f5c5d483340e`. Normal KROAD-010 records pin that ancestor; the lineage validator hard-codes and the policy enumerates the package, lockfile, both KROAD-010 gates, contract/schema, Decision Record schema, registry/rule, Resolver MVP, and L2 validator. Dedicated fixtures reject both a pin that predates the lineage stack and a pin whose execution bytes drift from the current acceptance stack.
